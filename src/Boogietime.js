...
export default class Boogietime {
  ...
  resume() {
    ...
    // if the player is stopped, but the queue is not empty, play the first track in the queue.
    // this can happen if the user clicks "add to queue" on a track while no music is playing.
    else if (this.state === 'stopped' && this.queue.length) {
  ...
  async next(_prevSongWasCompleted = false) {
    if (this.state === 'stopped') {
      console.log('Player is not playing anything')
      return
    }
  ...
  async playItemInQueue(index) {
    ...
    // change to playing, in case the music was previously paused
    this._stateChange('playing')

    // propagate track change event
    this._trackChange(queueItemID)

    this.createSystemNotification(queueItemID)
  ...
  /**
   * Returns the number of seconds elapsed in the current playback, or null if nothing is playing.
   * 
   * @returns {(Number|null)}
   */
  ...
  /**
   * Sets the shuffle mode to "true" or "false", and triggers "controlChange" callbacks.
   * 
   * @param {boolean} mode
   */
  setShuffle(mode) {
  ...
  /**
   * Adds one or more tracks to the end of the current queue. New playback cannot be started with this method,
   * use `play()` instead.
   * 
   * @param {(number|array}} trackIds - A track ID, or an array of track ID's
   * @param {number} [atIndex] - Optionally insert the new tracks at a certain index in the queue.
   */
  ...
  /**
   * Removes one track from the queue. Tracks may be located anywhere in the queue, even as
  ...
  /**
   * Removes all items from the queue (even previous items) except the currently playing item.
   * 
   ...
   * - `all`: clears all previous and all upcoming items from the queue. Leaves the currently playing
   * item as the only item in the queue.
   */
  clearQueue(mode = 'all') {
  ...
  /**
   * Allows custom elements to quietly update the queue without interrupting or changing
   * the currently playing track in any way. Does not trigger any callbacks. This exists
   * primarily to allow the playback-queue to update the Player queue after a user drags-
   * n-drops the queue tracks.
   * 
   * The currently playing track ID **must** be located somewhere in the newQueue array.
   * 
  ...
  /**
   * Begins playing the first item in the given queue, and saves the given queue internally.
   * 
  ...
  formatFilePath(path) {
    ...
    return filePathParts.join(Bridge.sep) + Bridge.sep + encodeURIComponent(fileName) // encodes characters like '#' in the file path name
  }

  /**
   * Uses a XHR request to check if the file for the song exists. XHR is preferrable over node fs because
   * this runs in a renderer process and because howler uses XHR as well.
   * 
   * @param {string} path - The file path on the local system. Only works in Electron, obviously.
   * @returns {boolean}
   */
  fileExists(path) {
  ...
  /**
   * Invoked by the public methods play(), pause(), etc, this will change the Player state inernally
   * and trigger appropiate registered callbacks.
   * 
   * @param {string} newState - The new state to become. `playing`, `paused`, `stopped`. Note: this will
   * not play/pause/stop the music, this will only refect that change internally after the main public method
   * performs the main play/pause/stop action.
   */
  ...
  /**
   * Invoked by the public methods prev() and next(), this is to propagate the track change to listeners.
   * 
